...
async def add_product(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        log.warning(f"Intento de agregar URL duplicada: {url}")
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("del_confirm_"):
        try:
            ...
            log.error(f"Error en del_confirm: {e}", exc_info=True)
    ...
    try:
        ...
        log.error(f"Error de split en callback_data: {data}")
        return
...
async def receive_target_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        log.error(f"Error en receive_target_price: {e}", exc_info=True)
...
def main():
    ...
    if not TELEGRAM_TOKEN or not CHAT_ID:
        log.critical("TELEGRAM_TOKEN o CHAT_ID no están en el archivo .env. Saliendo.")
        return
    ...
    log.info("El bot está escuchando comandos...")
...
if __name__ == "__main__":
    while True:
        try:
            ...
            log.warning(f"Error de red detectado: {e}. Reiniciando en 60 segundos...")
            time.sleep(60)
        except Exception as e:
            log.critical(f"Error fatal en el bot_manager: {e}", exc_info=True)
            log.info("Reiniciando en 5 minutos...")
            time.sleep(300)